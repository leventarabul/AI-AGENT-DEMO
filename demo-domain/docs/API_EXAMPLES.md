...
```bash
curl -X POST -u admin:admin123 http://localhost:8000/campaigns/2/rules \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -u admin:admin123 http://localhost:8000/events/3 | jq .
```
...
```bash
curl -u admin:admin123 'http://localhost:8000/admin/jobs/execution-logs?limit=5' | jq .
```
...
# 1. Create campaign
CAMPAIGN=$(curl -s -X POST -u admin:admin123 http://localhost:8000/campaigns \
  -H "Content-Type: application/json" \
...
# 2. Create rule
RULE=$(curl -s -X POST -u admin:admin123 http://localhost:8000/campaigns/$CAMPAIGN_ID/rules \
  -H "Content-Type: application/json" \
...
# 3. Register event
EVENT=$(curl -s -X POST -u admin:admin123 http://localhost:8000/events \
  -H "Content-Type: application/json" \
...
# 4. Process event
echo "Triggering job..."
curl -s -X POST -u admin:admin123 http://localhost:8000/admin/jobs/process-events | jq .

# 5. Check result
sleep 2
echo "Event after processing:"
curl -s -u admin:admin123 http://localhost:8000/events/$EVENT_ID | jq '.status, .matched_rule_id'

# 6. View logs
echo "Latest job execution:"
curl -s -u admin:admin123 'http://localhost:8000/admin/jobs/execution-logs?limit=1' | jq '.logs[0]'
```
